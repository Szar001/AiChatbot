...
class AuditLogStore:
    ...
    def record(self, actor: dict, action_type: str, summary: str,
               category: str = None, ticket_id: str = None, query_text: str = None) -> dict:
        ...
        try:
            cursor = conn.execute(
                ...
                INSERT INTO audit_log
                    (timestamp, actor_id, actor_name, actor_role, action_type, category, ticket_id, query_text, summary)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (timestamp, actor.get("id"), actor.get("name"), actor.get("role"),
                 action_type, category, ticket_id, query_text, summary),
            )
            conn.commit()
